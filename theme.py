...
/* Tiêu đề chữ gradient chuyển động, sang trọng, có hiệu ứng xuất hiện + lấp lánh nhẹ */
.edu-gradient-title {
    display: inline-block;
    font-weight: 800 !important;
    letter-spacing: 0.5px;
    background: linear-gradient(90deg, #3f6cf0, #8a5cf0, #4f9dff, #8a5cf0, #3f6cf0);
    background-size: 300% auto;
    -webkit-background-clip: text;
    background-clip: text;
    color: transparent;
    filter: drop-shadow(0 2px 10px rgba(94, 100, 220, 0.25));
    animation: edu-title-shine 6s linear infinite, edu-title-in 0.8s ease-out;
}
@keyframes edu-title-shine {
    0%   { background-position: 0% 50%; }
    100% { background-position: 300% 50%; }
}
@keyframes edu-title-in {
...
.edu-title-underline {
    ...
    margin: 6px auto 0;
    border-radius: 4px;
    background: linear-gradient(90deg, #4f7cff, #8a63f0);
    box-shadow: 0 2px 10px rgba(94, 100, 220, 0.35);
    animation: edu-underline-in 0.9s ease-out;
}
